...
export class OrderPreparingListComponent implements OnInit {
  ...
  async search() {
    ...
    let orders: any;
    console.log(`Date Picker Value ${this.searchDate}`)
    if (this.searchDate !== undefined) {
      orderCollection = this.afs.collection('orders', (ref) => ref.where('status', '==', 'preparing').where('time', '==', this.searchDate));
      this.searchStatus = true;
      this.pageNum = 1;
    } if(this.searchDate == null) {
      orderCollection = this.afs.collection('orders', (ref) => ref.where('status', '==', 'preparing').orderBy('time', 'desc'));
  ...
  nextPage() {
    if (this.searchStatus == true) {
      return this.afs.collection('orders', ref => {
        let query: Query = ref;
        query = query.where('status', '==', 'preparing').where('time', '==', this.searchDate).startAfter(this.lastSnapshot).limit(this.paginationLen);
        return query;
      });
    } else {
      return this.afs.collection('orders', ref => {
        let query: Query = ref;
        query = query.where('status', '==', 'preparing').orderBy('time', 'desc').startAfter(this.lastSnapshot).limit(this.paginationLen);
        return query;
      });
  ...
  previousPage() {
    if (this.searchStatus == true) {
      return this.afs.collection('orders', ref => {
        let query: Query = ref;
        query = query.where('status', '==', 'preparing').where('time', '==', this.searchDate).endAt(this.firstSnapshot).limitToLast(this.paginationLen);
        return query;
      });
    } else {
      return this.afs.collection('orders', ref => {
        let query: Query = ref;
        query = query.where('status', '==', 'preparing').orderBy('time', 'desc').endAt(this.firstSnapshot).limitToLast(this.paginationLen);
        return query;
      });
