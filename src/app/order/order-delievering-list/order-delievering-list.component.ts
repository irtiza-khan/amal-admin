...
export class OrderDelieveringListComponent implements OnInit {
  ...
  async search() {
    ...
    if (this.searchDate !== undefined) {
      orderCollection = this.afs.collection('orders', (ref) => ref.where('status', '==', 'delivering').where('time', '==', this.searchDate));
      this.searchStatus = true;
      this.pageNum = 1;
    } if (this.searchDate == null) {
      orderCollection = this.afs.collection('orders', (ref) => ref.where('status', '==', 'delivering').orderBy('time', 'desc'));
  ...
  nextPage() {
    if (this.searchStatus == true) {
      return this.afs.collection('orders', ref => {
        let query: Query = ref;
        query = query.where('status', '==', 'delivering').where('time', '==', this.searchDate).startAfter(this.lastSnapshot).limit(this.paginationLen);
        return query;
      });
    } else {
      return this.afs.collection('orders', ref => {
        let query: Query = ref;
        query = query.where('status', '==', 'delivering').orderBy('time', 'desc').startAfter(this.lastSnapshot).limit(this.paginationLen);
        return query;
      });
  ...
  previousPage() {
    if (this.searchStatus == true) {
      return this.afs.collection('orders', ref => {
        let query: Query = ref;
        query = query.where('status', '==', 'delivering').where('time', '==', this.searchDate).endAt(this.firstSnapshot).limitToLast(this.paginationLen);
        return query;
      });
    } else {
      return this.afs.collection('orders', ref => {
        let query: Query = ref;
        query = query.where('status', '==', 'delivering').orderBy('time', 'desc').endAt(this.firstSnapshot).limitToLast(this.paginationLen);
        return query;
      });
